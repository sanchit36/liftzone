...
export default function MeasuresScreen() {
    ...
    const handleAddMeasurement = () => {
        ...
        if (isNaN(val)) { Alert.alert('Invalid Value', 'Please enter a valid number.'); return; }
        if (val <= 0) { Alert.alert('Invalid Value', 'Value must be greater than zero.'); return; }
        if (val < addLimits.min || val > addLimits.max) {
            Alert.alert('Out of Range', `${MEASURE_LABELS[addType]} should be between ${addLimits.min} and ${addLimits.max} ${addLimits.unit}.`);
            return;
        }
        if (hasEntryToday(addType)) {
            Alert.alert('Already Recorded', `You've already added a ${MEASURE_LABELS[addType]} measurement today. Only one entry per day is allowed.`);
            return;
        }

        addMeasurement({ id: `m-${Date.now()}`, type: addType, value: val, unit: UNIT_MAP[addType], date: Date.now() });
        setAddValue('');
    ...
    const handleDeleteMeasurement = (id: string, value: number) => {
        Alert.alert('Delete Entry', `Remove ${value} ${unit} entry?`, [
            { text: 'Cancel', style: 'cancel' },
            { text: 'Delete', style: 'destructive', onPress: () => { removeMeasurement(id); setActivePoint(null); } },
        ]);
    };
        ...
        <SafeAreaView style={[s.container, { backgroundColor: c.background }]} edges={['top']}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.scroll}>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.tabs}>
                    {ALL_TYPES.map((type) => (
                        <TouchableOpacity
                            key={type}
                            style={[s.tab, { backgroundColor: c.card, borderColor: c.border }, selectedType === type && s.tabActive]}
                            onPress={() => { setSelectedType(type); setActivePoint(null); }}
                        >
                            <Text style={[s.tabText, { color: c.textSecondary }, selectedType === type && s.tabTextActive]}>
                                {MEASURE_LABELS[type]}
                            </Text>
                        </TouchableOpacity>
                    ))}
                ...
                {/* Chart Card */}
                <View style={[s.chart, { backgroundColor: c.card, borderColor: c.border }]}>
                    <View style={s.chartTop}>
                        <View>
                            <Text style={[s.chartTitle, { color: c.text }]}>{MEASURE_LABELS[selectedType]}</Text>
                            <Text style={[s.chartSub, { color: c.textSecondary }]}>
                                {activePoint !== null
                    ...
                    {data.length > 0 ? (
                            ...
                            {[0, 0.25, 0.5, 0.75, 1].map((pct, i) => {
                                ...
                                return (
                                    <React.Fragment key={i}>
                                        ...
                                        <SvgText x={PAD.left - 4} y={y + 3} fontSize={9} fill={c.textTertiary}
                            ...
                            {points.map((p, i) => (
                                <React.Fragment key={i}>
                                    <Circle cx={p.x} cy={p.y} r={activePoint === i ? 7 : 4}
                                        fill={activePoint === i ? Colors.primary : c.card}
                                        stroke={Colors.primary} strokeWidth={2}
                                        onPress={() => handleTapPoint(i)} />
                                    {(i === 0 || i === points.length - 1) && points.length > 1 && (
                                        <SvgText x={p.x} y={CHART_H - 8} fontSize={9} fill={c.textTertiary}
                        ...
                        <View style={s.emptyChart}>
                            <MaterialIcons name="show-chart" size={40} color={c.textTertiary} />
                            <Text style={[s.emptyChartText, { color: c.textTertiary }]}>Add a measurement to see chart</Text>
                        </View>
                    )}
                </View>

                {/* Measurement History */}
                <Text style={[s.historyTitle, { color: c.text }]}>History</Text>
                <View style={s.list}>
                    {[...data].reverse().map((m) => (
                        <View key={m.id} style={[s.row, { backgroundColor: c.card, borderColor: c.border }]}>
                            <View style={{ flex: 1 }}>
                                <Text style={[s.rowVal, { color: c.text }]}>{m.value} {m.unit}</Text>
                                <Text style={[s.rowDate, { color: c.textSecondary }]}>{formatDateLong(m.date)}</Text>
                            </View>
                            <TouchableOpacity style={s.deleteBtn} onPress={() => handleDeleteMeasurement(m.id, m.value)} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
                                <MaterialIcons name="delete-outline" size={18} color="#e74c3c" />
                            </TouchableOpacity>
                        </View>
                    ))}
                    {data.length === 0 && (
                        <Text style={[s.noData, { color: c.textTertiary }]}>No entries yet. Tap + to add one.</Text>
                    )}
                </View>
            ...
            <Modal visible={showAddModal} transparent animationType="fade">
                <TouchableOpacity style={s.overlay} activeOpacity={1} onPress={() => setShowAddModal(false)}>
                    <View style={[s.modal, { backgroundColor: c.background }]} onStartShouldSetResponder={() => true}>
                        <Text style={[s.modalTitle, { color: c.text }]}>Add Measurement</Text>

                        {/* Type Picker */}
                        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.modalTabs}>
                            {ALL_TYPES.map((type) => (
                                <TouchableOpacity key={type}
                                    style={[s.tab, { backgroundColor: c.card, borderColor: c.border }, addType === type && s.tabActive]}
                                    onPress={() => setAddType(type)}>
                                    <Text style={[s.tabText, { color: c.textSecondary }, addType === type && s.tabTextActive]}>
                                        {MEASURE_LABELS[type]}
                                    </Text>
                                </TouchableOpacity>
                            ))}
                        ...
                        {/* Limits hint */}
                        <Text style={[s.limitHint, { color: c.textTertiary }]}>
                        ...
                        <View style={s.inputRow}>
                            <TextInput
                                style={[s.modalInput, { color: c.text, backgroundColor: c.card, borderColor: c.border }]}
                                placeholder="0"
                                ...
                                autoFocus
                            />
                            <Text style={[s.inputUnit, { color: c.textSecondary }]}>{UNIT_MAP[addType]}</Text>
                        </View>
...
const s = StyleSheet.create({
    ...
    headerTitle: { fontSize: FontSize.lg, fontFamily: 'Lexend_700Bold' },
    ...
    tabText: { fontSize: FontSize.sm, fontFamily: 'Lexend_500Medium' },
    tabTextActive: { color: Colors.primary, fontFamily: 'Lexend_700Bold' },

    chart: { borderRadius: BorderRadius.lg, padding: Spacing.lg, borderWidth: 1 },
    chartTop: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: Spacing.base },
    chartTitle: { fontSize: FontSize.base, fontFamily: 'Lexend_600SemiBold' },
    chartSub: { fontSize: FontSize.sm, fontFamily: 'Lexend_400Regular' },
    ...
    emptyChartText: { fontFamily: 'Lexend_400Regular', fontSize: FontSize.sm },

    historyTitle: { fontSize: FontSize.lg, fontFamily: 'Lexend_700Bold', marginTop: Spacing.xl, marginBottom: Spacing.sm },
    list: { gap: Spacing.sm },
    row: {
        flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center',
        padding: Spacing.base, borderRadius: BorderRadius.sm, borderWidth: 1,
    },
    rowVal: { fontSize: FontSize.base, fontFamily: 'Lexend_600SemiBold' },
    rowDate: { fontSize: FontSize.xs, fontFamily: 'Lexend_400Regular', marginTop: 2 },
    deleteBtn: { padding: 8 },
    noData: { fontSize: FontSize.sm, fontFamily: 'Lexend_400Regular', textAlign: 'center', paddingVertical: Spacing.xl },
    ...
    addText: { fontSize: FontSize.base, fontFamily: 'Lexend_700Bold', color: '#000' },

    overlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'center', alignItems: 'center' },
    modal: { width: '85%', borderRadius: BorderRadius.lg, padding: Spacing.xl },
    modalTitle: { fontSize: FontSize.xl, fontFamily: 'Lexend_700Bold', marginBottom: Spacing.base, textAlign: 'center' },
    modalTabs: { gap: Spacing.sm, marginBottom: Spacing.sm },
    limitHint: { fontSize: FontSize.xs, fontFamily: 'Lexend_400Regular', textAlign: 'center', marginBottom: Spacing.sm },
    warningBanner: {
        flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: Spacing.sm,
        backgroundColor: 'rgba(230,126,34,0.15)', borderRadius: BorderRadius.sm, paddingVertical: 8, marginBottom: Spacing.sm,
    },
    ...
    modalInput: {
        flex: 1, fontSize: FontSize.xxxl, fontFamily: 'Lexend_700Bold',
        textAlign: 'center', borderRadius: BorderRadius.lg,
        paddingVertical: Spacing.base, borderWidth: 1,
    },
    inputUnit: { fontSize: FontSize.xl, fontFamily: 'Lexend_600SemiBold' },
    saveBtn: { backgroundColor: Colors.primary, borderRadius: BorderRadius.lg, paddingVertical: Spacing.base, alignItems: 'center' },
    saveBtnText: { fontSize: FontSize.base, fontFamily: 'Lexend_700Bold', color: '#000' },
});
